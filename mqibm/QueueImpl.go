...
func (queue QueueImpl) SetPutAsyncAllowed(paa int) Queue {

	// Check that the specified paa parameter is one of the values that we permit,
	// and if so store that value inside queue.
	if paa == Destination_PUT_ASYNC_ALLOWED_ENABLED ||
		paa == Destination_PUT_ASYNC_ALLOWED_DISABLED ||
		paa == Destination_PUT_ASYNC_ALLOWED_AS_DEST {
